...
async function pollSignatureOrderComplete(
  id: string,
  delay = 5000
): Promise<SignatureOrderQuery["signatureOrder"]> {
  ...
  const allComplete = signatureOrder.signatories.reduce(
    (complete, signatory) =>
      complete && signatory.status != SignatoryStatus.Open,
    true
  );
...
async function main() {
  ...
  const polledSignatureOrder = await pollSignatureOrderComplete(
  ...
  closedSignatureOrder.documents.forEach((document, index) => {
    writeFileSync(
      index
        ? __dirname + `/../output-sample-${index}.pdf`
        : __dirname + `/../output-sample.pdf`,
      Buffer.from(document.blob, "base64")
    );
  });

  console.log(
    "Signature order signed and closed, inspect result in output-sample.pdf"
  );
}
